...
QString Assignment::ModelID(void) {
  // specify the name of your model here
  return QString( " Group Assignment" );
}

// return the number of chemicals your model uses
int Assignment::NChem(void) { return 2; }

// To be executed after cell division
void Assignment::OnDivide(ParentInfo *parent_info, CellBase *daughter1, CellBase *daughter2) {
	// rules to be executed after cell division go here
	// (e.g., cell differentiation rules)
}

/*
    Introducing chemical 1 as a hormone that is produced by the attacked cells (cell type 1) in response to the pathogen attack (chemical 0), to strengthen the walls of nearby cells.
*/

void Assignment::SetCellColor(CellBase *c, QColor *color) {
	// add cell coloring rules here
    double col_inf = c->Chemical(0) / (0.2);
    double col_inf2 = c->Chemical(0) / (0.4);
    double col_inf3 = c->Chemical(1) / (0.5); // color more blue based on chemical 1
    if (col_inf > 1.0) {
    	col_inf = 1.0;
    }
    if (col_inf2 > 0.4) {
    ...
    color->setRgbF(0 + col_inf - col_inf2, 1 - col_inf2, 0 + col_inf3);
...
void Assignment::CelltoCellTransport(Wall *w, double *dchem_c1, double *dchem_c2) {
    ...
    if(stiffness>0.001){
        diffusionCoef = par->D[0]/stiffness;
    }
    else{diffusionCoef= par->D[0];}
...
void Assignment::WallDynamics(Wall *w, double *dw1, double *dw2) {
...
void Assignment::CellDynamics(CellBase *c, double *dchem) {
    ...
    // for all other cells, the chemical is degraded:
        ...
        dchem[1] = -0.01 * c->Chemical(1); // degradation of chemical 1 (signaling hormone)
    }
...
//Q_EXPORT_PLUGIN2(Assignment, Assignment)
